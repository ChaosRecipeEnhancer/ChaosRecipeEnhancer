...
using ChaosRecipeEnhancer.UI.WPF.BusinessLogic.Leagues;
using ChaosRecipeEnhancer.UI.WPF.BusinessLogic.StashTabs;
...
public abstract class ApiAdapter
{
	...
	public static bool FetchError
	{
		get; set;
	}
	private static bool FetchingDone
	{
		get; set;
	}
	private static StashTabListModel ListModel
	{
		get; set;
	}

	// legacytodo: [Refactor] Take a good look at this method - it's a bit redundant in nature. Lots of repeated API calls.
	...
	public static IEnumerable<string> FetchLeagueNamesAsync()
	{
		...
		using (var httpClient = new HttpClient())
		{
			try
			{
				var response = httpClient.GetAsync("https://api.pathofexile.com/leagues?type=main").Result;

				if (response.IsSuccessStatusCode)
				{
					var leagueJsonString = response.Content.ReadAsStringAsync().Result;
					var leagueList = JsonSerializer.Deserialize<List<LeagueModel>>(leagueJsonString);

					foreach (var league in leagueList)
					{
						leagueNames.Add(league.Id);
					}
				}
			}
