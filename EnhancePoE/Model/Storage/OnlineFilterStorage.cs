...
namespace EnhancePoE.Model.Storage
{
    public class OnlineFilterStorage : IFilterStorage
    {
        ...
        public async Task<string> ReadLootFilterAsync()
        {
            ...
            Trace.WriteLine("filter url", filterUrl);

            if (string.IsNullOrWhiteSpace(filterUrl))
            {
                MessageBox.Show("No loot filter found! Please check that you set the correct name and that you use your own filter. You cannot use followed filters.", "Could not read loot filter", MessageBoxButton.OK, MessageBoxImage.Error);
                return null;
            }
        ...
        {
            if (_filterData == null) await ReadLootFilterAsync();
        ...
        private async Task<string> GetFilterUrlAsync()
        {
            if (_filterUrl == null)
            {
                var url = string.Format(FiltersUrl.ToString(), _accName);
                ...
                using (var client = new HttpClient(handler))
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (_filterUrl != null) _filterUrl = "https://www.pathofexile.com" + _filterUrl;
                    }
                }
            }
