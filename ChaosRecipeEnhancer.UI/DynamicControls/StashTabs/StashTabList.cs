        ...
        public static void GetStashTabIndices()
        {
            if (Settings.Default.StashTabIndices != "")
            {
                ...
                foreach (var s in split)
                    ...
                    {
                        MessageBox.Show("StashTab Index has to be a number", "Error: Stash Tab Overlay",
                            MessageBoxButton.OK, MessageBoxImage.Error);
                    }

                if (ret.Count == 0)
                    MessageBox.Show("Stash Tab Indices empty", "Error: Stash Tab Overlay", MessageBoxButton.OK,
            ...
            {
                MessageBox.Show("No valid Stash Tab indices could be found in the user settings.",
                    "Error: Stash Tab Overlay", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
    }
}